    ...
    public partial class FrmPantallaInicial : Form
    {
        ...
        public void LoadCuentas()
        {
            try
            {
                ...
                {
                    int cantidad_final = Convert.ToInt32(row["CantidadFinal"]);
                    decimal precio_final = Convert.ToInt32(row["PrecioFinal"]);

                    Pedidos pedido = new Pedidos(row);
                    pedido.CantidadFinal = cantidad_final; 
                    pedido.PrecioFinal = precio_final;
